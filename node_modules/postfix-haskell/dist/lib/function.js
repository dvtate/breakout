...
class Fun {
     ...
     */
    constructor(token, condition, action, name, datatype) {
        this.tokens = [];
        this.conditions = [];
        this.actions = [];
    ...
    action(ctx, token) {
        ...
        ctx.stack = ctx.stack.map(v => v instanceof expr.DataExpr
        ...
        // Check types
        const typeErr = conds.find(rv => (rv instanceof value.DataValue || rv instanceof expr.DataExpr)
        ...
        const branches = (conds
            .map((ret, i) => !(ret instanceof Array || ret instanceof error.SyntaxError)
        ...
        const traceResults = branches
            ...
            .filter(io => io !== null);
        // Look for an error
        const err = traceResults.find(t => !(t instanceof context_1.TraceResults));
        if (err)
            return err;
        const ios = traceResults;
        ...
        // Make branch expr
        // TODO remove `as` here
        const branch = new expr.BranchExpr(this.tokens, branches.map(b => b[0]), ios.map(t => t.gives), this.name);
